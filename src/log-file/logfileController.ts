    ...
    try {
      ...
      extractedData.forEach((extracted) => {
        ...
        const regexTransactionCurrencyCode =
          /150.25:Transaction Currency Code = <(\d+)>/;
        ...
        //getting the objects
        const terminalApplicationVersionNumber =
          terminalApplicationVersionNumberMatch
            ? terminalApplicationVersionNumberMatch[1]
            : null;
        const vendor = vendorMatch ? vendorMatch[1] : null;
        const serialNumber = serialNumberMatch ? serialNumberMatch[1] : null;

        const deviceModel = deviceModelMatch ? deviceModelMatch[1] : null;
        const applicationLabel = applicationLabelMatch
          ? applicationLabelMatch[1]
          : null;
        const transactionStatusInformation = transactionStatusInformationMatch
          ? transactionStatusInformationMatch[1]
          : null;
        const transactionDate = transactionDateMatch
          ? transactionDateMatch[1]
          : null;

        const transactionCurrencyCode = transactionCurrencyCodeMatch
          ? transactionCurrencyCodeMatch[1]
          : null;

        const terminalVerificationResult = terminalVerificationResultMatch
          ? terminalVerificationResultMatch[1]
          : null;
        const terminalCountryCode = terminalCountryCodeMatch
          ? terminalCountryCodeMatch[1]
          : null;

        // const cryptogram = cryptogramMatch ? cryptogramMatch[1] : null;

        // const applicationUsageControl = applicationUsageControlMatch
        //   ? applicationUsageControlMatch
        //   : null;

        // const issuerActionCode = issuerActionCodeMatch
        //   ? issuerActionCodeMatch
        //   : null;
        // const interfaceDeviceSerialNumber = interfaceDeviceSerialNumberMatch
        //   ? interfaceDeviceSerialNumberMatch
        //   : null;

        // const terminalType = terminalTypeMatch ? terminalTypeMatch[1] : null;

        // const receivingInstitutionIdentificationCode =
        //   receivingInstitutionIdentificationCodeMatch
        //     ? receivingInstitutionIdentificationCodeMatch[1]
        //     : null;
        // const cardDataOutputCapability = cardDataOutputCapabilityMatch
        //   ? cardDataOutputCapabilityMatch[1]
        //   : null;
        // const amountOther = amountOtherMatch ? amountOtherMatch[1] : null;

        // const amountAuthorized = amountAuthorizedMatch
        ...
        //   : null;
        // const applicationIdentifier = applicationIdentifierMatch
        //   ? applicationIdentifierMatch[1]
        //   : null;
        ...
        newArray.push({
          ...
          // primaryAccountNumberSanitised,
          // terminalCardholderAuthenticationCapability,
          // uUIDS,
          // xAcquiringInstitutionIdCode,
          // cardholderAuthenticationEntity,
          // cardDataCaptureMethod,
        });

        return "regexTransactionCurrencyCode;";
      });
